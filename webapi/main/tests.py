...
class MainViewsTestCase(TestCase):
    def test_import(self):
        ...
        try:
            django.db.connections['default'] = old_db.__class__(
                old_db.settings_dict)
            ...
            # sheet
            selectString = "SELECT insee, dateobs,altitude_saisie, altitude_sig, altitude_retenue, supprime, pdop, the_geom_27572, the_geom_2154, saisie_initiale, id_organisme, srid_dessin, id_protocole, id_lot, the_geom_3857 FROM contactfaune.t_fiches_cf WHERE id_cf = %s" % (
                d.id_sheet)
            ...
            # statement 1
            selectString = "SELECT id_releve_cf, id_nom, id_critere_cf, am, af, ai, na, sai, jeune, yearling, cd_ref_origine, nom_taxon_saisi, commentaire, supprime, prelevement FROM contactfaune.t_releves_cf WHERE id_cf = %s and id_releve_cf = %s" % (
                d.id_sheet, ids_statements[0])
            cursor = query_db(selectString)
            assertDict = {
            ...
            for observer in inputData.observers_id:
                selectString = "SELECT count(*) FROM pr_occtax.cor_role_releves_occtax WHERE id_cf = %s AND id_role = %s" % (
                    d.id_sheet, observer)
                cursor = query_db(selectString)
                row = cursor.fetchone()
            ...
            deleteString = "DELETE FROM contactfaune.t_releves_cf WHERE id_cf = %s" % (
                d.id_sheet)
            query_db(deleteString)
            deleteString = "DELETE FROM contactfaune.t_fiches_cf WHERE id_cf = %s" % (
                d.id_sheet)
            query_db(deleteString)
            deleteString = "DELETE FROM pr_occtax.cor_role_releves_occtax WHERE id_cf = %s and id_role in (%s)" % (
                d.id_sheet, ','.join(map(str, inputData.observers_id)))
            query_db(deleteString)
            commit_transaction()
